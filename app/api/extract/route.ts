...
Before answering, verify that every session contains one record for every roster student, in the identical order. Return JSON only in this shape: {"sheet":{"school_name":null,"class_name":null,"day_time":null,"lead_coach":null,"teacher":null,"assistant":null,"theme":null,"focus_training":null,"rules":null,"around_world":null,"notes":null},"students":["name"],"sessions":[{"date":"YYYY-MM-DD","metrics":["metric"],"records":[{"student_name":"name","attendance":"present|absent|late|unknown","values":{"metric":"raw cell text"},"confidence":0.0}]}]}.`;
  ...
  if(confirmedThreeDateTemplate){
   ...
   const focusedImage=await sharp(prepared).extract({left:Math.round(preparedWidth*.385),top:Math.round(preparedHeight*.06),width:Math.round(preparedWidth*.145),height:Math.round(preparedHeight*.54)}).resize({width:1600}).sharpen().jpeg({quality:94}).toBuffer();
   const focusedData=focusedImage.toString("base64");
   const focusedPrompt=`This crop contains exactly the ${firstDate} attendance column followed by exactly two score columns: Focus Training and Right Behavior. Read all three cells for each of the 14 filled rows from top to bottom. Normalize P to present and A to absent. P and A must never appear as score values. Score values may be numerals, dashes, or blank. Return exactly 14 rows and JSON only: {"metrics":["Focus Training","Right Behavior"],"rows":[{"row_number":1,"attendance":"present|absent|unknown","values":{"Focus Training":"raw score","Right Behavior":"raw score"},"confidence":0.0}]}. Blank score cells are empty strings.`;
   ...
   const absentHasNumericScore=Array.isArray(focused.rows)&&focused.rows.some((row:{attendance?:string;values?:Record<string,string>})=>row.attendance==="absent"&&Object.values(row.values||{}).some(value=>/\d/.test(String(value))));
   ...
   extractedSessions[0].records=extractedSessions[0].records.map((record:{confidence?:number},index:number)=>({...record,attendance:focused.rows[index]?.attendance||"unknown",values:focused.rows[index]?.values||{},confidence:Math.min(record.confidence??1,focused.rows[index]?.confidence??1)}));
